(function(){
  ...
  View.registerHelper('partial', function(name, options) {
    this._view.partials || (this._view.partials = {});
    this._view.partials[name] = this._view[name];
    this._view[name] = function(callOptions) {
      ...
      var output = this.partials[name].call(this, wrapPartialOptionsBlockCallback(optionsForCallback));
      this.$('[' + partial_attribute_name + '="' + name + '"]').html(output);
    }
    options.hash[partial_attribute_name] = name;
    return new Handlebars.SafeString(View.tag(options.hash, this._view.partials[name].call(this._view, wrapPartialOptionsBlockCallback(options)), this));
  });
