(function(){
  ...
  _.extend(View.prototype, Backbone.View.prototype, {
    ...
    render: function(output) {
      if (typeof output === 'undefined' || (!_.isElement(output) && !is$(output) && !(output && output.el) && typeof output !== 'string')) {
        output = this.template(this._template || getViewName.call(this), this._getContext(this.model));
      } else if (typeof output === 'function') {
        output = this.template(output, this._getContext(this.model));
      }
  ...
  View.registerHelper('collection', function(collection, options) {
    ...
    if (collection) {
      ...
      return new Handlebars.SafeString(View.tag.call(this, collectionHelperOptions));
    } else {
      return '';
    }
  });
  ...
  View.tag = function(attributes, content) {
    ...
    return '<' + tag + ' ' + _.map(htmlAttributes, function(value, key) {
      return key + '="' + Handlebars.escapeExpression(value) + '"';
    }).join(' ') + '>' + (content || '') + '</' + tag + '>';
  };
